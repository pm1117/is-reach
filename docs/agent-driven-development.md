...
```text
is-reach-orchestrator に従って進めて。
いまはフェーズ3（詳細設計）のみ。PR分割・実装は禁止。
docs/requirements.md / docs/basic-design.md は承認済み前提。
architect（software-architecture）と ui-designer（frontend-design）に委譲し、
docs/design-detail.md と docs/ui-spec.md を作成する。
進め方:
1. まず基本設計の決定（D1〜D8）と仮置き一覧を要約し、詳細設計で決める論点を列挙する
2. 論点は1〜2個ずつ質問形式で確認する（勝手に全部決め切らない）
3. 未確定は「仮置き」、確定は「決定」と区別する
4. 合意できた範囲で docs/design-detail.md（API・プロンプト・エラー等）と
   docs/ui-spec.md（画面遷移・主要UI）の草案を書く
5. 終わったら承認ポイントを箇条書きで報告し、私の OK を待つ
詳細設計で必ず含めること（design-detail.md）:
- API 契約（エンドポイント一覧、リクエスト/レスポンス型、認可マトリクス）
- プロンプト構成（system / user / 外部データブロックの分離、サニタイズ・出力検証ルール）
- エラー状態・リトライ方針（深掘りジョブ、クローリング、LLM呼び出し）
- 基本設計の仮置きの確定または再仮置き
  （認証サービス、pg-boss、Claudeモデル、レート制限具体値、
   メッセージ生成の同期/非同期、深掘り進捗通知、テンプレ権限、削除方針 等）
- SQL/RLS の方針レベル詳細（ポリシー方針・主要インデックス。実装DDL全文は任意）
- 監査ログのイベント網羅リスト・保持期間
詳細設計で必ず含めること（ui-spec.md）:
- 画面一覧と画面遷移（スクリーニング → リスト → ドシエ → メッセージ編集）
- 主要 UI コンポーネント方針（粒度・命名、Tailwind トークン方針）
- 状態表現（ローディング / 空 / エラー / 権限なし / 生成中）
- 生成文は必ず人手確認・編集できること（自動送信なし）
- 外部由来テキスト表示時のエスケープ・サニタイズ前提
書かないこと:
- 実装コード
- PR分割計画（次フェーズ）
- 基本設計で既に決定したパッケージ境界・ドメインモデルの再発明（参照のみ）
```
