...
| 遷移 | 条件 |
|---|---|
| `collecting → failed` | 全ページ取得失敗（トップページ含め 1 ページも取得できない）: `CRAWL_ALL_FAILED` / collecting フェーズタイムアウト（**5 分**） |
...
| 429 | `Retry-After` を尊重して **1 回だけ**再試行。以後そのドメインへの間隔を当該ジョブ内で 2 倍に緩和 | 1 回 |
...
| レート制限具体値 | **確定（提案値 → 決定案）** | 同一ドメイン **1 req/秒**（最小間隔 1,000ms）・同一ドメイン同時 **2 接続**・クローラープロセス全体同時 **5 接続**・深掘り 1 サイト上限 **20 ページ**・シグナル収集 1 ソース上限 **5 ページ**・1 ページタイムアウト 15 秒・本文上限 2MB。User-Agent: `is-reach-bot/<version> (+<運用サイトの bot 説明ページ URL>; contact: <連絡先>)`（連絡先の具体値は**仮置き** — 運用ドメイン確定時） |
...
| E9 | 深掘りジョブ詳細 | 専用ジョブレコード方式、部分失敗許容（1 ページ以上で analyzing へ）、自動リトライ 2 回・タイムアウト 10 分等（→ 4.1） |
...
| E12 | クローリング節度の具体値 | 同一ドメイン 1 req/秒・同時 2 接続・全体 5 接続・深掘り 1 サイト 20 ページ 等（→ 5 章） |
...
- [ ] 本詳細設計書の承認（承認者: ）
- [ ] （確認）前フェーズ `docs/basic-design.md` が承認済みであること（2026-07-13 承認済み）
