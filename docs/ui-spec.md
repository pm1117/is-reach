...
- ダッシュボード（S1）は**仮置き**: MVP では「最近のリスト」「進行中の深掘りジョブ」「ステータス集計（未着手/生成済み/送信済み/返信あり の件数）」の 3 ブロックの簡易版とし、実装優先度は最後尾。工数次第で初回リリースから外し `/lists` をログイン後の着地点にしてもよい。
...
| U2 | 画面構成 | 2.2 の S0〜S9 の 10 画面と遷移（ダッシュボードの中身・要否は仮置き） |
...
| 項目 | 仮置き内容 |
|------|-----------|
| ダッシュボード（S1） | 3 ブロック簡易版。MVP から外し `/lists` 着地でも可 — 承認時に判断 |
...
- [ ] 本 UI 仕様書の承認（承認者: ）
- [ ] （確認）ダッシュボード（S1）の扱い: 簡易版を MVP に含める / MVP から外す（どちらかを選択）
